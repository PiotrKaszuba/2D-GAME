...
void Room::drop_bonus(World<Physical>* mapa, Sprite_Sheet_Data ** graphics, float multiplier)
{
	...
	case 0:
		...
		this->feature_data.real_value = 5*multiplier;
		break;
	case 1:
		this->feature_data.integer_value = 1;
		this->feature_data.real_value = 0.0025 * multiplier;
		break;
	case 2:
		this->feature_data.integer_value = 3;
		this->feature_data.real_value = 2 * multiplier;
		break;
	}
	...
	mapa->Set_objects(-1);
	this->feature = new Bonus_Holder(8, *graphics[12], mapa, this->feature_data.position, (bonus_type)this->feature_data.integer_value, this->feature_data.real_value);
}
...
void Room::initialize(World<Physical> *mapa, Cube<Room> *cube, Sprite_Sheet_Data **graphics, float multiplier)
{
	...
	if (!this->room_initialized)
	{
		...
		{
		case 5:
			this->feature_data.real_value = 100;
			break;
		}
	...
	if (this->feature_alive)
	{
		...
		case 4:
			...
			break;
		case 5:
			this->feature = new Enemy(8, *graphics[11], mapa, mapa->get_obstacle()[rand() % 6 + 1]->getPosition(),this->feature_data.real_value, 10*multiplier);
			break;
		case 10:
			this->feature = new Bonus_Holder(8, *graphics[12], mapa, this->feature_data.position, (bonus_type)this->feature_data.integer_value, this->feature_data.real_value);
			break;
