...
Damage_Trap_Spreader::Damage_Trap_Spreader(int index_range_start, int number_of_traps, Sprite_Sheet_Data trap_sample, World<Physical> *mapa, int trap_lifetime, float trap_damage) : Room_Feature()
{
	...
	{
		this->traps[i].trap = new Damage_Trap(i+this->index_range_start, trap_sample, sf::Vector2f(float(rand() % (mapa->getSize().x - mapa->Get_border_size().x) + mapa->Get_border_size().x / 2), float(rand() % (mapa->getSize().y - mapa->Get_border_size().y) + mapa->Get_border_size().y / 2)), mapa, this->trap_damage);
...
void Damage_Trap_Spreader::update(World<Physical>* mapa)
{
	...
	for (int i = 0; i < number_of_traps;i++)
	{
		...
		if (this->shape == random)
			this->traps[i].trap->setPosition(sf::Vector2f(float(rand() % (mapa->getSize().x - mapa->Get_border_size().x) + mapa->Get_border_size().x / 2), float(rand() % (mapa->getSize().y - mapa->Get_border_size().y) + mapa->Get_border_size().y / 2)));
