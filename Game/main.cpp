...
6. Boots icon:
Icon made by Iconka.com from http://www.iconarchive.com/show/lucky-leprechaun-icons-by-iconka/boot-icon.html
License: http://www.iconarchive.com/icons/iconka/lucky-leprechaun/license.txt

7. Blue medical (regen icon)
Icon made by MedicalWP from http://www.iconarchive.com/show/medical-icons-by-medicalwp/Health-Sign-blue-icon.html
License: https://creativecommons.org/licenses/by/4.0/


8. Red fist icon:
Icon made by Mattahan from http://www.iconarchive.com/show/ultrabuuf-icons-by-mattahan/Comics-Rulk-Fist-icon.html
License: https://creativecommons.org/licenses/by-nc-sa/4.0/

-----------------------------------------------------------------------
Public Domain:
  9. Fire trap particles
  10. Roman column
...
int main()
{
	...
	Player y(b, mapa, sf::Keyboard::Left, sf::Keyboard::Right, sf::Keyboard::Up, sf::Keyboard::Down, sf::Keyboard::LControl, sf::Keyboard::Z, sf::Keyboard::Space, sf::Keyboard::E, sf::Keyboard::Escape, sf::Keyboard::Tab);
...
sf::Texture *LOAD_texture()
{
	...
	sf::Image imdamage;
	imdamage.loadFromFile(name_image_damage);

	sf::Image imregen;
	imregen.loadFromFile(name_image_regen);

	sf::Image imspeed;
	imspeed.loadFromFile(name_image_speed);
	...
	imdamage.createMaskFromColor(sf::Color(255, 255, 255, 255));

	imregen.createMaskFromColor(sf::Color(255, 255, 255, 255));

	imspeed.createMaskFromColor(sf::Color(255, 255, 255, 255));
	...
	textures[5].loadFromImage(imdamage);

	textures[6].loadFromImage(imregen);

	textures[7].loadFromImage(imspeed);
	

	textures[8].loadFromImage(imredfloor);


	textures[9].loadFromImage(imgreyfloor);


	textures[10].loadFromImage(imyellowfloor);


	textures[11].loadFromImage(imexitborder);
	

	textures[12].loadFromImage(imexitmiddle);
	

	textures[13].loadFromImage(imladder);


	textures[14].loadFromImage(imcolumn);
...
Sprite_Sheet_Data **LOAD_graphics(sf::Texture *textures)
{
	...
	Sprite_Sheet_Data *damage = new Sprite_Sheet_Data;
	Sprite_Sheet_Data *regen = new Sprite_Sheet_Data;
	Sprite_Sheet_Data *speed = new Sprite_Sheet_Data;
	...
	Sprite_Sheet_Data **graphics = new Sprite_Sheet_Data*[graphic_set];
	...
	damage->width = 42; damage->height = 46;
	regen->width = 46; regen->height = 48;
	speed->width = 44; speed->height = 40;
	...
	damage->still = Type_of_Animation(1, sf::Vector2i(0, 0), sf::Vector2i(0, 0), sf::Vector2i(6, 2));
	regen->still = Type_of_Animation(1, sf::Vector2i(0, 0), sf::Vector2i(0, 0), sf::Vector2i(2, 0));
	speed->still = Type_of_Animation(1, sf::Vector2i(0, 0), sf::Vector2i(0, 0), sf::Vector2i(4, 8));
	...
	*background_y = *background_r;
	...
	sf::Sprite xsdamage = sf::Sprite(textures[5], sf::IntRect(0, 0, damage->width, damage->height));

	sf::Sprite xsregen = sf::Sprite(textures[6], sf::IntRect(0, 0, regen->width, regen->height));

	sf::Sprite xsspeed = sf::Sprite(textures[7], sf::IntRect(0, 0, speed->width, speed->height));

	sf::Sprite xsredfloor =sf::Sprite(textures[8], sf::IntRect(0, 0, 256, 256));

	sf::Sprite xsgreyfloor = sf::Sprite(textures[9], sf::IntRect(0, 0, 256, 256));

	sf::Sprite xsyellowfloor = sf::Sprite(textures[10], sf::IntRect(0, 0, 256, 256));

	sf::Sprite xsexitborder = sf::Sprite(textures[11], sf::IntRect(0, 0, 64, 64));

	sf::Sprite xsexitmiddle = sf::Sprite(textures[12], sf::IntRect(0, 0, 64, 64));

	sf::Sprite xsladder = sf::Sprite(textures[13], sf::IntRect(0, 0, 128, 128));

	sf::Sprite xscolumn = sf::Sprite(textures[14], sf::IntRect(0, 0, 256, 640));
	// set scales
