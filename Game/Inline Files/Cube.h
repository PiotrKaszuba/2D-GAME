...
inline Cube<x>::Cube(char sum, char divisibility, char permutation, char does_not_contain, Sprite_Sheet_Data **graphics, World<Physical> *mapa, int time)
{
	...
	this->timer.setFillColor(sf::Color::Yellow);
	this->time = time;
...
inline void Cube<x>::timer_update(World<Physical> *mapa)
{
	...
	if (c > 10) a += 10;
	if (d > 10) a += 10;
	float damage = mapa->get_obstacle()[0]->get_damage();
	float regen = mapa->get_obstacle()[0]->get_health_regen() * 20;
	float speed = mapa->get_obstacle()[0]->get_moving_speed();
	float health = mapa->get_obstacle()[0]->get_health();
	float max_health = mapa->get_obstacle()[0]->get_max_health();
	this->timer.setString(std::string("      ")
		.append(std::to_string(b)).append(std::string(":")).append(std::to_string(c)).append(std::string(":")).append(std::to_string(d))
		.append("\n\n\nHealth: ").append(to_string_with_precision(health,0)).append("/").append(to_string_with_precision(max_health,0))
		.append("\n Damage: ").append(to_string_with_precision(damage))
		.append("\n Regen:  ").append(to_string_with_precision(regen))
		.append("\n Speed:  ").append(to_string_with_precision(speed))
		);
	this->timer.setPosition(mapa->getSize().x / 2 - a - 85, mapa->getSize().y / 2 - 120.f);
...
inline void Cube<x>::intro(World<Physical> *mapa){
	...
	case 'p':
		...
		info[4] = "Poruszasz sie przy pomocy klawiszy strzalek, walka piesciami - LCTRL, kopniak - SPACE, blok - Z, uzycie/wejscie - E, pozostaly czas/pomoc - TAB, automatyczne odczekanie do konca czasu - ESCAPE.";
	...
	case 'e' :
		...
		info[4] = "You can move by pressing arrow keys, fight with arms - LCTRL, kick SPACE, block - Z, use/enter - E, time left/help - TAB, automatic wait to the end of time - ESCAPE.";
